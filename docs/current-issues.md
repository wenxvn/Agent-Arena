...
### 4. 14B 已安装，但仍未通过自动逃生验收

现在 `qwen2.5:14b` 已安装（约 9 GB），模型连接验证通过。固定 seed=0、MemoryAgent、30 步的真实运行中，模型在第 14 步恢复主电源后，连续重复读取诊断终端，最终达到步数上限。

因此 14B 解决了“模型未安装”的问题，但没有解决“长程任务会循环”的问题。
...
| 自动完成逃生 | 未通过 | 7B 和 14B 都出现重复动作，达到 30 步上限 |
...
4. 若目标是演示“必定通关”，应让确定性规划器负责任务阶段，模型只负责局部动作；这会改变研究变量，需要另立方案。
