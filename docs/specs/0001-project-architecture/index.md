...
| Decision provider interface | `DecisionProvider` protocol with Ollama, OpenAI-compatible Responses, Bailian compatibility and Fake implementations | The Runner receives one injected decision provider, so the same Agent Loop supports deterministic tests and explicit real model runs. |
...
|---|---|---|---|---|
| `run` | `--provider fake` is the default. `--provider ollama`, `--provider openai` or `--provider bailian` is explicit. | No positional input in the first version. | Defaults to `spaceship-escape`, `react`, seed `0`, and `runs/`. It writes one episode JSON file. | `0` after a trace is written, even for a task failure. Configuration or runner failures return nonzero. |
| `benchmark` | `--provider fake` 默认为 Fake；必须显式指定 `--provider ollama`、`--provider openai` 或 `--provider bailian`。 | 局数必须为正数。 | 默认在 `spaceship-escape` 中以相同 seed 运行 `react` 与 `memory` 对照；seed 从 `0` 确定性递增，结果写入 `results/`。`--agent react` 或 `--agent memory` 可选择单一 Agent。输出一个 benchmark JSON 和一个 CSV。 | 所有请求 outcome 持久化后返回 `0`。配置无效或无法启动时返回非零。 |
| `verify-model` | `--provider ollama`、`--provider openai` 或 `--provider bailian`，不会回退为 Fake。 | Ollama 本地服务和模型，或有效的 OpenAI-compatible/Bailian 端点和 API key。 | Prints only the configured model name and final text. | `0` only for a successful compatible response. Configuration, authentication, network, or protocol failure returns nonzero without printing sensitive content. |

`--provider openai` requires `OPENAI_BASE_URL` and `OPENAI_API_KEY`, and calls the compatible `/v1/responses` endpoint with structured JSON output. `OPENAI_MODEL` selects the remote model. `--provider bailian` remains a compatibility path for the existing Chat Completions adapter and accepts `OPENAI_API_KEY` or `DASHSCOPE_API_KEY`. Fake runs require no endpoint or key.
