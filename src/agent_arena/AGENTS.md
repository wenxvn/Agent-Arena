...
- `run` 通过 `ReactAgent`、`MemoryAgent` 或明确标记的 `PlannerAssistedAgent`、`EpisodeRunner` 和 `write_episode_trace` 运行完整 episode。`benchmark` 使用持久化 trace 生成 JSON、CSV 对照指标；`llm/` 适配 Fake、百炼和本地 Ollama，且不得让任何 Agent 或 Environment 直接调用 SDK；Spaceship Escape 环境位于 `arena/` 和 `worlds/`。
